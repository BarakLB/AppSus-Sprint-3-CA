...
function addBook(book) {
    ...
    const jsonBook = bookFromAPItoJSON(book)
    console.log(jsonBook)
    books.unshift(jsonBook)
    _saveBooksToStorage(books)
    return Promise.resolve();
}

function bookFromAPItoJSON(book) {

    const { id } = book;
    const { title, subtitle, authors, description, publishedDate, pageCount, categories } = book.volumeInfo;
...
// id,title,subtitle,authors, description, publishedDate,pageCount, categories, thumbnail, listPrice:amount,isOnSale, currencyCode


// console.log(getGoogleBooks('american sniper').then(console.log))
...
function createReview(review, id) {
    ...
    books[bookIdx]['reviews'].push(_createReview(review));
    _saveBooksToStorage(books);
    console.log(review)
}
